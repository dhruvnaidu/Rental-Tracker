...
// Firebase Initialization and Authentication
    ...
    useEffect(() => {
      ...
      const unsubscribeProperties = onSnapshot(userPropertiesCollectionRef, async (snapshot) => {
        ...
        for (const doc of snapshot.docs) {
          ...
          const unitsCollectionRef = collection(db, `artifacts/${__app_id}/users/${doc.id}/units`); // Corrected path
  ...
  const MonthlyRentTracker = () => {
    ...
    const handleSelectAllRentRecords = () => {
      if (selectedRentRecords.length === filteredRentRecords.length && filteredRentRecords.length > 0) {
    ...
    const handleSelectAllArrearsRecords = () => {
      if (selectedArrearsRecords.length === arrearsRecords.length && arrearsRecords.length > 0) {
